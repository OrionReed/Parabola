...
public class ZippyTerrain2DEditor : Editor {
	...
	private void OnSceneGUI() {
		...
		for (int i = 0; i < tar.terrainCurve.length; i++) {
			...
			Vector2 mPos = Handles.FreeMoveHandle(v, Quaternion.identity, .5f * tar.handleScale, Vector3.zero, Handles.CircleHandleCap);
			if(i > 0 && i < tar.terrainCurve.length-1) {
				Handles.color = Color.red;
				if (shiftDown && Handles.Button(v, Quaternion.identity, .8f * tar.handleScale, 1 * tar.handleScale, Handles.SphereHandleCap)) {
			...
			if (ctrlDown && Handles.Button(v, Quaternion.identity, .8f * tar.handleScale, 1 * tar.handleScale, Handles.SphereHandleCap)) {
			...
			if (i < tar.terrainCurve.length - 1) {
				...
				Handles.color = Color.white;
				if (Handles.Button(vvv2, Quaternion.identity, .4f * tar.handleScale, .4f * tar.handleScale, Handles.CubeHandleCap)) {
